...
items_data = {
    ...
    "دسته‌بندی": random.choices(categories, weights=category_weights, k=num_items),
    "رنگ": random.choices(colors, weights=color_weights, k=num_items),
    "متریال": random.choices(materials, weights=material_weights, k=num_items),
    "سبک طراحی": random.choices(design_styles, weights=design_style_weights, k=num_items),
    "کاربرد": random.choices(usages, weights=usage_weights, k=num_items),
    "جنسیت": random.choices(genders, weights=gender_weights, k=num_items),
}
